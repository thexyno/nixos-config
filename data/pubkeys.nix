let
  ragon =
    let
      pc = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMvexOT9tnx2LfAE/OwfixfNc/esNAjZ+GDfLpY2iABk"
        "ssh-rsa " # root
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKXtH/ZY7u7ejf+EyzWleWRVUP8aNU5Gna5lpfVPRcuj" # hostkey
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKejPH5g1z8Syx5YhypidjMZ6itJTgDBBpfAVUIb4+a5" # voyager hostkey
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGGyWUwZv6aRrdMxxIu6lRS44C1fJu/l+UknAGx57okd" # wormhole hostkey
      ];
      work = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEJwOH1b6xWmEr1VZh48kBIYhW11vtPFR3my8stAHlSi"
      ];
    in
    {
      inherit pc work;
      computers = pc ++ work;
    };
in
{
  inherit ragon;
}
